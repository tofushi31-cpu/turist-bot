...
def build_visa_exempt_text() -> str:
    return next(
        text for oid, label, highlight, text in VISA_INFO["turkey"]["options"] if oid == "exempt"
    )
...
def build_confirmation_email_body(booking: dict) -> str:
    return (
        ...
        "Если вы впервые в Турции или ещё не оформляли визу — вот актуальная информация о безвизовом въезде:\n\n"
...
async def handle_visa_docs(callback: CallbackQuery):
    info = VISA_INFO["turkey"]
    options = [(opt_id, label) for opt_id, label, _, _ in info["options"]]
    await callback.message.answer(
        f"{info['label']}: выберите вариант въезда", reply_markup=build_choice_menu(options, "visaopt_turkey")
    )
    await callback.answer()
